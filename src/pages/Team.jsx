      ...
      <motion.div className="w-full py-10 px-4 lg:px-8" initial="hidden" animate="visible">
         ...
         <motion.div className="flex flex-wrap justify-center gap-4" variants={fadeIn}>
            {teamMembers.slice(0, 2).map((member, idx) => (
               <motion.div 
                  key={idx} 
                  className="text-center w-[220px] bg-white/80 p-4 rounded-lg shadow-md hover:shadow-lg cursor-pointer transform transition-transform duration-300 hover:-translate-y-1 border border-gray-200/50 backdrop-blur-lg flex flex-col items-center"
         ...
         {/* 3 Members (Below) */}
         <motion.div className="flex flex-wrap justify-center gap-4 mt-4" variants={fadeIn}>
            {teamMembers.slice(2, 5).map((member, idx) => (
               <motion.div 
                  ...
                  onClick={() => setSelectedMember(member)}
                  whileHover={{ scale: 1.05 }}>
                  <img className="mb-3 w-18 h-18 rounded-full border-2 border-gray-300" src={member.imageUrl} alt={member.name} />
                  <h3 className="text-md font-semibold text-gray-900">{member.name}</h3>
                  <p className="text-sm text-gray-600">{member.designation}</p>
               </motion.div>
            ))}
